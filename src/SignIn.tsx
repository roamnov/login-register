...
export function getURLparam(key: any) {
  const urlParams = new URLSearchParams(window.location.search);
  const param: any = urlParams.get(key);
  return param;
}

export function hasURLparam(key: any) {
  const urlParams = new URLSearchParams(window.location.search);
...
export default function SignIn() {
  ...
  const [modalContent, setModalContent] = React.useState<any>("0"); // 1- каптча ; 2-почта;3 - текст
  ...
  React.useEffect(() => {
    if(hasURLparam("reason")){
      const reason:string = getURLparam("reason")
      if(hasURLparam(reason)){
        const errorFromUrl:string = getURLparam(reason)
